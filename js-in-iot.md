...
当我们的硬件层不能直接联网时，我们的协调层就可以完成这样的功能。作为一个协调层的设备，它应该能与一定数量的微控制器连接，接收它们的数据，并上传到服务端；又能与服务端通讯，获取一些控制指令，并将这些指令准备地发送给不同的控制器。因此，它需要有更好的处理能力、更多的 RAM、ROM 等等。
...
如上所述，物联网应用的架构与 Web 应用的架构区别并不大。当前在 Web 开发中流行的一些元素，势必也会引导到物联网系统中：
